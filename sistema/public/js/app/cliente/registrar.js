(function () {
	...
	formulario_registro.addEventListener("submit", function (e) {
		...
		if (c_identificacion.value == "") {
			let texto_id = c_tipo_identificacion.value == "C" ? "Cédula" : "RIF";
			Toast.fire({ icon: "error", title: `¡Ingrese el número de ${texto_id}!` });
			c_identificacion.focus();
		} else if (c_tipo_identificacion.value == "C" && c_identificacion.value.length < 8) {
			Toast.fire({ icon: "error", title: "¡La cédula está incompleta!" });
			c_identificacion.focus();
		} else if (c_tipo_identificacion.value == "R" && c_identificacion.value.length < 10) {
			Toast.fire({ icon: "error", title: "¡El RIF está incompleto!" });
			c_identificacion.focus();
		} else if (c_nombre_completo.value == "") {
			Toast.fire({ icon: "error", title: "¡Ingrese el Nombre/Razon social del cliente!" });
			c_nombre_completo.focus();
		} else if (c_nombre_completo.value.length < 10) {
			Toast.fire({ icon: "error", title: "¡El Nombre/Razon social debe tener al menos 10 caracteres!" });
			c_nombre_completo.focus();
